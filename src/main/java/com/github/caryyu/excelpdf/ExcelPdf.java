 ...
 */
public class ExcelPdf extends PdfTool{
     ...
     * @param object
     */
    public ExcelPdf(ExcelObject object , OutputStream os) {
     ...
     * @param objects
     */
    public ExcelPdf(List<ExcelObject> objects , OutputStream os) {
    ...
    public void convert() throws DocumentException, MalformedURLException, IOException {
        ...
        if(this.objects.size() <= 1){
        	List<PdfPTable> tables = this.toCreatePdfTable(this.objects.get(0) ,  getDocument() , writer);
        ...
        if(this.objects.size() > 1){
            ...
            for (int i = 0; i < this.objects.size(); i++) {
            	List<PdfPTable> tables = this.toCreatePdfTable(this.objects.get(i) , getDocument() , writer);
            	for (PdfPTable table : tables) {
            		table.setKeepTogether(true);
            		table.getDefaultCell().setBorder(PdfPCell.NO_BORDER);
            		getDocument().add(table);
    ...
    protected List<PdfPTable> toCreatePdfTable(ExcelObject object , Document document , PdfWriter writer) throws MalformedURLException, IOException, DocumentException{
